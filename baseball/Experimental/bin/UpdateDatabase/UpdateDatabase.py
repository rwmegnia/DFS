...
for date in schedule["dates"]:
    ...
    for game in date["games"]:
        ...
        away_batting_slice = away_team_batting_stats[TEAM_BATTING_SHARE_STATS]
        away_batting_slice = away_batting_slice.add_prefix("team_")
        home_batting_slice = home_team_batting_stats[TEAM_BATTING_SHARE_STATS]
    ...
    opp_pitchers = pitchers[pitchers.gamesStarted == 1]
    opp_pitchers.drop("team", axis=1, inplace=True)
    opp_pitchers.rename(
        {
            "opp": "team",
            "player_id": "opp_pitcher_id",
            "throws": "opp_pitcher_hand",
        },
        axis=1,
        inplace=True,
    )
    batters = batters.merge(
        opp_pitchers[["team", "opp_pitcher_id", "opp_pitcher_hand"]],
        on=["team"],
        how="left",
    )
    ...
    batters.to_csv(f"{datadir}/game_logs/batterstatsDatabase.csv", index=False)
    pitchers.to_csv(
    ...
    at_bats.to_csv(f"{datadir}/game_logs/HittingStatsDatabase.csv", index=False)
#%%
for contest_type in ["Classic"]:
    dirs = os.listdir(f"../../data/Projections/RealTime/2022/{contest_type}")
    for game_date in dirs:
        if game_date=='.DS_Store':
            continue
        files = os.listdir(
            f"../../data/Projections/RealTime/2022/{contest_type}/{game_date}"
        )
        for f in files:
            if ".csv" not in f:
                continue
            df = pd.read_csv(
                f"../../data/Projections/RealTime/2022/{contest_type}/{game_date}/{f}"
            )
            df.drop_duplicates(inplace=True)
            if "DKPts" not in df.columns.to_list():
                if game_date in batters.game_date.unique():
                    pitchers = pitchers[pitchers.game_date == game_date]
                    batters = batters[batters.game_date == game_date]
                    # Do a check for two-way player Shohei Ohtani
                    if "Shohei Ohtani" in pitchers.full_name.to_list():
                        batters = batters[batters.full_name != "Shohei Ohtani"]
                    results = pd.concat(
                        [
                            batters,
                            pitchers,
                    ...
                    df = df.merge(
                        results[["player_id", "team", "DKPts", "game_id"]],
                        on=["player_id", "team", "game_id"],
                        how="left",
                    )
