...
GAME_TYPE = "R"
print(season)
schedule = getSeasonSchedule(season, gameType=GAME_TYPE)
...
for date in schedule["dates"]:
    ...
    skaters["Roster Position"] = skaters.position.apply(
        lambda x: x[-1] + "/UTIL"
    )
    ...
    try:
        ...
        skaters.loc[
            skaters.powerPlayTimeOnIceRank <= 10, "PowerPlayLine"
        ] = True
...
for contest_type in ["Classic", "Showdown"]:
    ...
    for game_date in dirs:
        ...
        for f in files:
            if ("Analysis" in f)|('.DS_Store' in f):
                continue
            df = pd.read_csv(
            ...
            if "DKPts" not in df.columns.to_list():
                if game_date in skaters.game_date.unique():
                    ...
                    df = df.merge(
                        results[["player_id", "DKPts", 'shots', 'blocked']],
                        on="player_id",
                        how="left",
                    )
