...
class Model:
    def __init__(self, pos_type, line, target="DKPts", window=5):
        self.target = target
        self.line = line
        self.window = window
        ...
        if self.pos_type in ["Forward", "Defenseman"]:
            ...
            self.Database['value']=(self.Database.DKPts/self.Database.Salary)*1000

            self.Database = self.Database[
                (self.Database.position_type == self.pos_type)
                & (self.Database.line == self.line)
                & (self.Database.timeOnIce>self.Database.timeOnIce.quantile(0.10))
            ]
    ...
    def fitModel(self, method, X_train=None, y_train=None, pipe=False, k=None):
        ...
        if pipe == True:
            pipe = make_pipeline(PCA(n_components=10), regressor)
            regressor = pipe
