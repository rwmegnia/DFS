...
class Model:
    def __init__(self, target="DKPts", window=15):
        ...
        self.Database = self.Database[self.Database.DKPts > 0]
        self.Database.sort_values(by=["team", "game_date"], inplace=True)
    ...
    def defenseFeatures(self, database):
        database = database.groupby("team").apply(
            lambda x: self.standardizeStats(x)
        )
        defenseFeatures = (
