...
def processRankings(proj, game_date, powerplay=True, Scaled=True):
    ...
    proj["ShotRank"] = proj.ShotBonusProb.rank(ascending=False, method="max")
    ...
    if Scaled==True:
        proj["Projection"] = proj[
            ["Stochastic", "RG_projection", "ML", "TD_Proj", "ScaledProj", "PMM"]
        ].mean(axis=1)
    else:
        proj["Projection"] = proj[
            ["Stochastic", "RG_projection", "ML", "TD_Proj", "PMM"]
        ].mean(axis=1)
