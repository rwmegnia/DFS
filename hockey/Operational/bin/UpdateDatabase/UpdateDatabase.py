...
## Read in master databases that we'll be updating
...
for date in schedule["dates"]:
    ...
    for game in date["games"]:
        ...
        away_team_stats = pd.DataFrame.from_dict(away_team["teamStats"]).T.reset_index(
            drop=True
        )
        ...
        home_team_stats = pd.DataFrame.from_dict(home_team["teamStats"]).T.reset_index(
            drop=True
        )
        ...
        home_skater_stats = home_skater_stats.merge(home_slice, on="team", how="left")
        away_skater_stats = away_skater_stats.merge(away_slice, on="team", how="left")
        # Get Goalie Stats
    ...
    try:
        ...
        if len(skaters[skaters.Salary.isna() is False]) == 0:
            continue
...
for contest_type in ["Classic", "Showdown"]:
    dirs = os.listdir(f"../data/Projections/RealTime/2021/{contest_type}")
    for game_date in dirs:
        files = os.listdir(
            f"../data/Projections/RealTime/2021/{contest_type}/{game_date}"
        )
        for f in files:
            df = pd.read_csv(
                f"../data/Projections/RealTime/2021/{contest_type}/{game_date}/{f}"
            )
            if "DKPts" not in df.columns.to_list():
                if game_date in skaters.game_date.unique():
                    results = pd.concat(
                        [
                            skaters[skaters.game_date == game_date],
                            goalies[goalies.game_date == game_date],
                        ]
                    )
                    df = df.merge(
                        results[["player_id", "DKPts"]], on="player_id", how="left"
                    )
                    df["Rank"] = df.DKPts.rank(ascending=False, method="min")
                    df = processRankings(df, game_date, powerplay=False)
                    df.to_csv(
                        f"../data/Projections/RealTime/2021/{contest_type}/{game_date}/{f}",
                        index=False,
                    )
