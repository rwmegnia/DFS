...
def RBStochastic(players, stats_df):
    ...
    cluster_model=KMeans(n_clusters=2).fit(stats_df[['spread_line','total_line']])
...
def MLPrediction(stats_df,weekly_proj_df,stat_type):
    ...
    matchup_model=KMeans(n_clusters=2).fit(stats_df[['proj_team_score','total_line','spread_line']])     
...
def TeamStatsPrediction(off_db,schedule):
    ...
    off_db.sort_values(by='game_date',inplace=True)
    matchup_model=KMeans(n_clusters=2).fit(off_db[['ImpliedPoints','total_line','spread_line']])
