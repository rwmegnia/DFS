...
model=NN().fit(temp[['start_yard_line']],temp.kick_distance)
pickle.dump(model,open(f'{model_dir}/PuntModels/PuntDistance_model.pkl','wb'))
model=NN().fit(temp[(temp.kick_distance.isna()==False)&(temp.punt_result.isin(['fair_catch','touchback','out_of_bounds','muffed']))][['kick_distance']],temp[(temp.kick_distance.isna()==False)&(temp.punt_result.isin(['fair_catch','touchback','out_of_bounds','muffed']))].elapsed_time)
...
model=NN().fit(temp[['kick_distance','return_yards']],temp.elapsed_time)
...
#%%
# Punt Def Fumble Recovery Yards
...
temp.loc[temp.block_recovered_by==temp.posteam,'block_recovered_by']='posteam'
temp.loc[temp.block_recovered_by==temp.defteam,'block_recovered_by']='defteam'
...
temp['block_recovered_at_territory']=temp.block_recovered_at.apply(lambda x: x.split(' ')[0] if type(x)==str else x)
temp.loc[temp.block_recovered_at_territory==temp.posteam,'block_recovered_at_territory']='posteam'
temp.loc[temp.block_recovered_at_territory==temp.defteam,'block_recovered_at_territory']='defteam'
temp['block_recovered_at'] = temp.block_recovered_at.apply(lambda x: x.split(' ')[1] if type(x)==str else x)
temp.loc[temp.block_recovered_at_territory=='defteam','block_recovered_at']=100-temp.loc[temp.block_recovered_at_territory=='defteam','block_recovered_at']
temp['block_recovered_at']=temp.block_recovered_at.astype(float)
...
temp['block_returned_to_territory'] = temp.block_returned_to.apply(lambda x: x.split(' ')[0] if type(x)==str else x)
temp.loc[temp.block_returned_to_territory==temp.posteam,'block_returned_to_territory']='posteam'
temp.loc[temp.block_returned_to_territory==temp.defteam,'block_returned_to_territory']='defteam'
temp['block_returned_to']=temp.block_returned_to.apply(lambda x: x.split(' ')[1] if (type(x)==str) & (len(str(x).split(' '))>1) else x)
temp.loc[temp.block_returned_to_territory=='defteam','block_returned_to']=100-temp.loc[temp.block_returned_to_territory=='defteam','block_returned_to']
temp['block_returned_to']=temp.block_returned_to.astype(float)
#
temp['territory']=temp.block_returned_to_territory
temp['end_yard_line']=temp.block_returned_to
...
## Punt Block Result
model=NN().fit(temp[['start_yard_line']],temp.punt_block_result)
pickle.dump(model,open(f'{model_dir}/PuntModels/PuntBlockResult_model.pkl','wb'))

temp.block_recovered_by.fillna('posteam',inplace=True)

## Who Recovered Punt 1 for defense 0 for possession team
temp.loc[temp.block_recovered_by=='posteam','block_recovered_by']=False
temp.loc[temp.block_recovered_by=='defteam','block_recovered_by']=True

model=DC(strategy='stratified').fit(temp[['qtr']],temp.block_recovered_by)
pickle.dump(model,open(f'{model_dir}/PuntModels/PuntBlockRecoveredBy_model.pkl','wb'))

## Where was punt recovered relative to line of scrimmage
model = NN().fit(temp[temp.block_recovered_at.isna()==False][['start_yard_line']],
                 temp[temp.block_recovered_at.isna()==False].block_recovered_at)
pickle.dump(model,open(f'{model_dir}/PuntModels/PuntBlockRecoveredAt_model.pkl','wb'))
## Elapsed Time Models
model=NN().fit(temp[temp.punt_block_result=='touchdown'][['start_yard_line']],temp[temp.punt_block_result=='touchdown'].elapsed_time)
pickle.dump(model,open(f'{model_dir}/PuntModels/PuntBlockTD_elapsed_time_model.pkl','wb'))

model=NN().fit(temp[temp.punt_block_result=='safety'][['start_yard_line']],temp[temp.punt_block_result=='safety'].elapsed_time)
pickle.dump(model,open(f'{model_dir}/PuntModels/PuntBlockSafety_elapsed_time_model.pkl','wb'))

model=NN().fit(temp[temp.punt_block_result=='punt_block_out_of_bounds'][['start_yard_line']],temp[temp.punt_block_result=='punt_block_out_of_bounds'].elapsed_time)
pickle.dump(model,open(f'{model_dir}/PuntModels/PuntBlockOB_elapsed_time_model.pkl','wb'))

model=NN().fit(temp[temp.punt_block_result=='block_returned'][['start_yard_line','block_return_yards']],temp[temp.punt_block_result=='block_returned'].elapsed_time)
pickle.dump(model,open(f'{model_dir}/PuntModels/PuntBlockReturn_elapsed_time_model.pkl','wb'))

# Punt Block Return Yards Model
temp=temp[temp.punt_block_result=='block_returned']
model=NN().fit(temp[['punt_blocked','block_recovered_by','block_recovered_at']],temp.block_return_yards)
