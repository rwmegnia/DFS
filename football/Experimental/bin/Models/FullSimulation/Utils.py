...
def getClass(classes, probs):
    ...
    probs = probs[probs > 0]
    Class = np.random.choice(classes,replace=False,p=probs)
    return Class
...
def retrieveSackFeatures(game):
    ...
    sack_features.fillna(0,inplace=True)
...
def runModel(features,model):
    probs = model.predict_proba([features])[0]
    classes=model.classes_
    result = np.random.choice(classes,p=probs)
    return result

def exportStats(df,i):
    passing_data = df.groupby(['passer_player_id','posteam']).agg(
        {'pass_attempt':np.sum,
         ...
         'iteration':'first'})
    iteration=passing_data.iteration.unique()[0]
    home_team_score=passing_data.home_team_score.unique()[0]
    away_team_score=passing_data.away_team_score.unique()[0]
    passing_data=passing_data.reset_index().rename({'passer_player_id':'gsis_id',
                                       'posteam':'team'},axis=1).set_index(['gsis_id','team'])
    rushing_data = df.groupby(['rusher_player_id','posteam']).sum()[
        ['rush_attempt',
         'rushing_yards',
         'rush_touchdown']]
    rushing_data=rushing_data.reset_index().rename({'rusher_player_id':'gsis_id',
                                                    'posteam':'team'},axis=1).set_index(['gsis_id','team'])
    receiving_data = df.groupby(['receiver_player_id','posteam']).sum()[
        ['pass_attempt',
         'complete_pass',
         'passing_yards',
         'pass_touchdown']
        ]
    receiving_data.rename({'pass_attempt':'targets',
                           'complete_pass':'rec',
                           'passing_yards':'rec_yards',
                           'pass_touchdown':'rec_touchdown'},axis=1,inplace=True)
    receiving_data=receiving_data.reset_index().rename({'receiver_player_id':'gsis_id',
                                                        'posteam':'team'},axis=1).set_index(['gsis_id','team'])
    
    df = passing_data.join(rushing_data,how='outer').join(receiving_data,how='outer')
    ...
    df.loc[df.passing_yards>=300,'fpts']+=3
    df.loc[df.rushing_yards>=100,'fpts']+=3
    df.loc[df.rec_yards>=100,'fpts']+=3
    df['iteration']=iteration
    df['home_team_score']=home_team_score
    df['away_team_score']=away_team_score
    df.to_csv(f'SimStats_{i}.csv')