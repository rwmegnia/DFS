...
# optimizer.add_stack(PositionsStack(["QB", "WR",("RB","WR","TE")]))
...
#%%
maes=proj.mean()[['RotoGrinders_mae',
                  'FantasyPros_mae',
                  'FP_Expert_Cons_mae',
                  'GridIron_mae',
                  'Nick_mae',
...
ax.set_ylim(maes.min()-.05,maes.max()+.05,0.5)
ax.set_yticks(np.arange(maes.min()-.05,maes.max()+.05,0.05))
for x,y in enumerate(maes):
    ax.annotate(y,(x-.05,y+.01))

plt.title(f'Week {week} MAE N = {len(proj)} Players')
