...
proj.drop(proj[(proj.position=='QB')&
               (proj.depth_team>1)].index,inplace=True)
...
proj.drop(proj[(proj.position.isin(['K','DST']))&
               (proj['Roster Position']=='CPT')].index,inplace=True)

proj['ID']=proj.ID.astype(int)
K=proj[proj.position=='K']
