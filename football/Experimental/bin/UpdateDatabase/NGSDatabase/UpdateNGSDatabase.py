...
def getNGSData(season):
    ...
    return ngs
#%%
def updateNGSDatabase(season):
    ngs = getNGSData(season)
    ngs.to_csv(f'{datadir}/NextGenStats/{season}/NextGenStats_{season}.csv',index=False)
    # Update Master Database
    db=pd.read_csv(f'{datadir}/NextGenStats/2012_present_offensive_snapcounts.csv')
    db=db[db.season!=season]
    db=pd.concat([db,ngs])
    db.to_csv(f'{datadir}/NextGenStats/NextGenStats_2016_present.csv',index=False)