...
def rolling_average(df, window):
    return df.rolling(min_periods=int(window/2), window=window).mean()
...
def QBStochastic(players, stats_df):
    ...
    low = pd.DataFrame(
        np.quantile(sims, 0.15, axis=0), columns=DK_Stoch_stats
    ).set_index(averages.gsis_id)
    low.rename({"DKPts": "Floor1"}, axis=1, inplace=True)
    low["Floor"] = getDKPts(low, "Offense")
    low["Floor"] = low[["Floor", "Floor1"]].mean(axis=1)
    high = pd.DataFrame(
        np.quantile(sims, 0.85, axis=0), columns=DK_Stoch_stats
    ).set_index(averages.gsis_id)
    high["Ceiling"] = getDKPts(high, "Offense")
    high["Ceiling"] = high[["Ceiling", "DKPts"]].mean(axis=1)
    stoch = pd.concat(
...
def WRStochastic(players, stats_df):
    ...
    return offense
#%%
def DefenseStochastic(weekly_proj_df, stats_df, contestType='Classic'):
    ...
    low = pd.DataFrame(
        np.quantile(sims, 0.15, axis=0), columns=DK_DST_stats
    ).set_index(averages.team)
    low.rename({"DKPts": "Floor1"}, axis=1, inplace=True)
    low["Floor"] = getDKPts(low, "defense")
    low["Floor"] = low[["Floor", "Floor1"]].mean(axis=1)
    high = pd.DataFrame(
        np.quantile(sims, 0.85, axis=0), columns=DK_DST_stats
    ).set_index(averages.team)
...
def rookiePrediction(weekly_proj_df, stats_df):
    ...
    if len(rookie_df[rookie_df.position!='DST'])!=0:     
...
def PlayerSharesPrediction(stats_df, weekly_proj_df):
    ...
    ProjFrame=weekly_proj_df[NonFeatures+KnownFeatures+['matchup']]
...
def TeamStatsPrediction(off_db, schedule):
    ...
    features.fillna(0, inplace=True)
    opp_features = off_db.groupby("opp").apply(
        lambda x: rolling_average(x, window).drop(
            KnownFeatures + NonFeatures, errors="ignore", axis=1
        )
    )
    opp_features = opp_features.add_suffix("_allowed")
...
def TeamDepthChartModelPredict(week, season, schedule, odds, seasontype):
    ...
    game_date = getGameDate(week, season, seasontype)
...
def TeamStatsModelPredict(week, season, schedule, odds, seasontype):
    sdf = schedule.merge(
        odds[["team", "proj_team_score", "total_line", "spread_line"]],
        on="team",
        how="left",
