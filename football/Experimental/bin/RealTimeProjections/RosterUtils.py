...
def filterRosterData(df, season, week):
    ...
    df.loc[df.position == "DST", "gsis_id"] = df.loc[df.position == "DST"].index
    df.loc[df.full_name=='Randall Cobb','injury_status']='Active'
...
def getWeeklyRosters(season, week, schedule, odds, game_date, season_type):
    ...
    df.loc[df.full_name.isin(['Khalil Herbert',
                              'Andy Isabella',
                              'Elijah Mitchell',
                              'John Brown',
                              'Tony Jones',
    ...
    injuries=getWeeklyInjuries(week,season,season_type)
...
def getGameDate(week, season,seasontype='REG'):
    schedule = getSchedule(week, season,seasontype=seasontype)
    if seasontype=='POST':
        week-=18
...
def getGameDates(week, season, seasontype='POST'):
    schedule = getSchedule(week, season, seasontype=seasontype)
    schedule.game_day = pd.to_datetime(schedule.GameDate)
