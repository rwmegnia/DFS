...
class Model:
    if 'matchup' in KnownFeatures:
        KnownFeatures.remove('matchup')
    def __init__(self,target='QB1_DKPts',window=8,team=False):
        self.window=window
        self.target=target
        self.team=team
    ...
    def offenseFeatures(self,database):
        # offenseFeatures=database.groupby(['team']).apply(lambda x: rolling_average(x,window=self.window)).drop(NonFeatures+KnownFeatures,axis=1,errors='ignore')    
        offenseFeatures=database.groupby(['team','matchup']).apply(lambda x: rolling_average(x,window=self.window)).drop(NonFeatures+KnownFeatures,axis=1,errors='ignore')    
    ...
    def defenseFeatures(self,database):
        # defenseFeatures=database.groupby(['opp','season','week']).sum().drop(NonFeatures+KnownFeatures,axis=1,errors='ignore')    
        defenseFeatures=database.groupby(['opp','season','week']).sum().drop(NonFeatures+[c for c in KnownFeatures if c!='matchup'],axis=1,errors='ignore')    
