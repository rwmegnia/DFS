...
def ScrapeBettingOdds(week,season,game_date):
    print("Retrieving Betting Lines...")
    teams = pd.DataFrame(columns=["team"])
    soup = BS(
        requests.get("https://rotogrinders.com/nfl/odds").content, "html.parser"
    )
    # Scrape Teams
    ...
    for link in soup.find_all(
        ...
        spread = spread.append(
            pd.DataFrame({"spread": [val1]}), ignore_index=True
        )
        spread = spread.append(
            pd.DataFrame({"spread": [val2]}), ignore_index=True
        )
        # if i == len(teams):
        #     break
    ...
    try:
        old_df=pd.read_csv(f"{datadir}/BettingOdds/{season}_Week{week}_BettingOdds.csv")
        if old_df.game_date.unique()[0]!=game_date:
            return old_df
        df=pd.concat([old_df,df])
        df=df.groupby('team',as_index=False).last()
    except FileNotFoundError:
        pass
    df.to_csv(f"{datadir}/BettingOdds/{season}_Week{week}_BettingOdds.csv", index=False)
    return df
