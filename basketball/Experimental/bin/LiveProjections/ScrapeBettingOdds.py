...
def ScrapeBettingOdds():
    ...
    teams = pd.DataFrame(columns=["team_abbreviation"])
    soup = BS(requests.get("https://rotogrinders.com/nba/odds").content, "html.parser")
    # Scrape Teams
    for link in soup.find_all(attrs={"class": "row game"}):
        home = link.get("data-team-home")
        away = link.get("data-team-away")
        teams = teams.append(pd.DataFrame({"team_abbreviation": [away]}), ignore_index=True)
        teams = teams.append(pd.DataFrame({"team_abbreviation": [home]}), ignore_index=True)
    team_frames = []
    books = len(
    ...
    df.rename({"total": "total_line",
               "spread":"spread_line"}, axis=1, inplace=True)
    df.team_abbreviation.replace(
        {
            "PHO": "PHX",
            "SA":"SAS",
            "NO":"NOP",
    ...
    df = df.groupby("team_abbreviation", as_index=False).mean()
    df['game_date']=game_date
    odds_db=pd.concat([odds_db,df])
    odds_db=odds_db.groupby(['game_date','team_abbreviation'],as_index=False).last()
